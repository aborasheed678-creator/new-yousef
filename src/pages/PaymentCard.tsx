...
const PaymentCard = () => {
  ...
  const { colors: primaryColors, logo: entityLogo } = brandingData;
  ...
  const displayImage = brandingData.background_image || branding.ogImage;
        ...
        <div className="min-h-screen bg-indigo-50 p-4">
            <div className="max-w-md mx-auto bg-white rounded-3xl shadow-xl overflow-hidden">
                <div className="bg-indigo-900 p-6 text-white">
                    <h2 className="text-xl font-bold">دفع حجز {link.payload.chalet_name}</h2>
                    <p className="text-indigo-200 text-sm">أكمل بيانات الدفع الآمنة</p>
                </div>
                <div className="p-6">
                    {/* Chalet Card Form UI */}
                    <Button onClick={handleSubmit} className="w-full bg-indigo-800">تأكيد الدفع</Button>
                </div>
            </div>
        </div>
    );
