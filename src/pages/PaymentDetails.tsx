...
import { detectEntityFromURL, getEntityLogo, getEntityIdentity, getBranding } from "@/lib/dynamicIdentity";
import PageLoader from "@/components/PageLoader";
import { applyChaletTheme } from "@/lib/utils";
...
const PaymentDetails = () => {
  ...
  useEffect(() => {
    if (linkData?.type === 'chalet' && linkData?.payload?.chalet_name) {
      applyChaletTheme(linkData.payload.chalet_name);
    }
  }, [linkData]);
  ...
  const brandingData = getBranding(serviceKey, linkData?.type);
  const { colors: primaryColors, logo: entityLogo, name: entityName } = brandingData;
  const displayLogo = entityLogo || branding.logo;
  const primaryColor = primaryColors.primary;
  const secondaryColor = primaryColors.secondary;
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-white">
                ...
                <p className="text-xs opacity-90">
                  {linkData?.type === 'chalet' ? 'نظام الحجز الآمن - Secure Booking' : 'الدفع الآمن - Secure Payment'}
                </p>
              </div>
            </div>
              ...
              <>
                ...
                >
                  <div className="flex items-center gap-3">
                    {linkData?.type === 'chalet' ? (
                        <div className="w-full bg-indigo-900 text-white p-6 rounded-2xl shadow-2xl">
                            <h2 className="text-2xl font-bold mb-2">حجز شاليه فاخر</h2>
                            <p className="text-indigo-200">أهلاً بك في {linkData?.payload?.chalet_name}</p>
                            <img src={`/assets/chalets/${(linkData?.payload as any)?.chalet_name?.replace(/\s/g, '_')}.jpg`} alt="Chalet" className="w-full h-40 object-cover mt-4 rounded-xl" onError={(e) => e.currentTarget.src = '/assets/chalets/default.jpg'} />
                        </div>
                    ) : (
                        // ... (keep existing shipping UI)
                        <div 
                    ...
                    {linkData?.type !== 'chalet' && (
                      ...
                      <h2 className="text-xl font-bold" style={{ color: designSystem.colors.neutral[900] }}>
                        {linkData?.type === 'chalet' ? (linkData?.payload as any)?.chalet_name : 'تفاصيل الشحنة'}
                      </h2>
                      <p className="text-sm text-gray-600">
                        {linkData?.type === 'chalet' ? 'بيانات الحجز' : 'معلومات الطرد والتوصيل'}
                      </p>
                    </div>
                    )}
                  </div>
